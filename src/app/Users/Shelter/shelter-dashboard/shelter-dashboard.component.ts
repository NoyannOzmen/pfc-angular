...
export class ShelterDashboardComponent {
  ...
  ShelterProfileForm = new FormGroup({
    nom: new FormControl({ value: this.user.refuge?.nom, disabled: true }, [
      Validators.minLength(3),
      Validators.required,
    ]),
    responsable: new FormControl({ value: this.user.refuge?.responsable, disabled: true }, [
      Validators.minLength(3),
      Validators.required,
    ]),
    rue: new FormControl({ value: this.user.refuge?.rue, disabled: true }, [
      Validators.minLength(3),
      Validators.required,
    ]),
    commune: new FormControl({ value: this.user.refuge?.commune, disabled: true }, [
      Validators.minLength(3),
      Validators.required,
    ]),
    code_postal: new FormControl({ value: this.user.refuge?.code_postal, disabled: true }, [
      Validators.pattern(/^(?:0[1-9]|[1-8]\d|9[0-8])\d{3}$/),
      Validators.required,
    ]),
    pays: new FormControl({ value: this.user.refuge?.pays, disabled: true }, [
      Validators.minLength(3),
      Validators.required,
    ]),
    telephone: new FormControl({ value: this.user.refuge?.telephone, disabled: true }, [
      Validators.pattern(/^(0|\+33 )[1-9]([-. ]?[0-9]{2} ){3}([-. ]?[0-9]{2})|([0-9]{8})$/),
    ]),
    siret: new FormControl({ value: this.user.refuge?.siret, disabled: true }, [
      Validators.pattern(/^(\d{14}|((\d{3}[ ]\d{3}[ ]\d{3})|\d{9})[ ]\d{5})$/),
      Validators.required,
    ]),
