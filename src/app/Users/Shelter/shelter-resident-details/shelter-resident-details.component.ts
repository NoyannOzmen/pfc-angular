...
export class ShelterResidentDetailsComponent {
  ...
  upload(event: Event) {
    const toUpload = event.target as HTMLInputElement;
    const file: FileList | null = toUpload.files;
    if (file) {
      this.userShelterService.updateAnimalPicture(file[0], this.animalId);
    }
  }
